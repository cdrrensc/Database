...
\begin{enumerate}[A)]
	...
	\item We have an outer-query which seeks gold medalist in s sport that appears in the nested query, which computes all sports that have appeared only once
	\item
	\item We unite (UNION) two same query using "Summer" for one and "Winter" for the other and compute the number of medals for each country given the type (Summer or Winter), we then order them despondingly and limit the table to 1
